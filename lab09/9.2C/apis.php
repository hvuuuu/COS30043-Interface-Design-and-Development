...
$conn = mysqli_connect('feenix-mariadb.swin.edu.au', 's104177995', '180804', 's104177995_db');
...
// Validate cp field to ensure it is a number
if (isset($input['cp']) && !is_numeric($input['cp'])) {
    header('Content-Type: application/json');
    echo json_encode(['error' => 'The cp field must be a number.']);
    mysqli_close($conn);
    exit();
}
